// Arranque y orquestación: estado, router, ubicaciones, GPS, overlay de día y SW.
import {state,activeLocation,agroOn} from './state.js';
...
import {initRouter,register,refreshCurrent,go,pushOverlay,closeOverlay} from './router.js';
import {initGeoWatch,checkGeoOnce,syncGeoWatch} from './geo-watch.js';
import {toast} from './toast.js';
import {fa} from './fa.js';
import {miniMapHTML} from './minimap.js';
import {renderDayDetail} from './sections/dia-detalle.js';
...
function navButtons(){
  return navDefs().map(([name,mod])=>`
    <button data-nav="${name}" class="navb">
      ${fa(mod.navIcon,{size:19})}<span>${mod.label}</span>
    </button>`).join('');
}
function buildNav(){
  $('bottomnav').innerHTML=navButtons();
  $('sidenav').innerHTML=navButtons();
  syncNavActive();
  document.documentElement.classList.toggle('no-agro',!agroOn());
}
function syncNavActive(){
  document.querySelectorAll('[data-nav]').forEach(b=>{
    const on=b.dataset.nav===state.section;
...
async function loadWeather(locationId,{silent=false}={}){
  ...
  showLoading(false);
  if(locationId===state.activeLocationId){syncHeader();refreshCurrent();}
}
...
function syncHeader(){
  const loc=activeLocation();
  const name=loc?loc.label:'Elegí ubicación';
  $('location-name').textContent=name;
  $('location-name-rail').textContent=name;
  const gps=!!(loc&&loc.isGPS);
  $('gps-pill').hidden=!gps;
  $('gps-pill-rail').hidden=!gps;
  $('gps-pill').classList.toggle('live',!!state.geo.watching);
  $('gps-pill-rail').classList.toggle('live',!!state.geo.watching);
}
function showLoading(on){
  $('loading-overlay').hidden=!on;
  document.querySelectorAll('.refresh-btn').forEach(b=>b.classList.toggle('spinning',on));
}
  ...
  list.innerHTML=state.locations.map(loc=>`
    <div class="loc-row${loc.isGPS?' loc-row-gps':''}${loc.id===state.activeLocationId?' active':''}" data-id="${loc.id}">
      <button class="loc-select">
        <span class="loc-name">${loc.isGPS?fa('crosshairs'):flagEmoji(loc.countryCode)} ${loc.label}</span>
        <span class="loc-sub">${[loc.admin1,loc.country].filter(Boolean).join(', ')}</span>
      </button>
      ${loc.isGPS?'':`<button class="loc-remove" title="Quitar">${fa('x')}</button>`}
...
// ── Overlay de detalle de día ────────────────────────────
function openDayDetail(dayIdx){
  state.dayDetailIdx=dayIdx;
  $('daydetail-panel').innerHTML=renderDayDetail(dayIdx);
  $('daydetail-overlay').hidden=false;
  pushOverlay('daydetail');
  requestAnimationFrame(()=>$('daydetail-overlay').classList.add('visible'));
}
function closeDayDetail({fromPop=false}={}){
  if(!$('daydetail-overlay').classList.contains('visible'))return;
  $('daydetail-overlay').classList.remove('visible');
  setTimeout(()=>{$('daydetail-overlay').hidden=true;},200);
  if(!fromPop)closeOverlay();
}

// ── Wiring ───────────────────────────────────────────────
buildNav();
initRouter({view:$('view'),onNavigate:()=>{syncNavActive();}});

document.querySelectorAll('.location-btn').forEach(b=>b.addEventListener('click',openLocationModal));
...
document.querySelectorAll('.refresh-btn').forEach(b=>b.addEventListener('click',refreshActive));
...
$('daydetail-close').addEventListener('click',()=>closeDayDetail());
$('daydetail-overlay').addEventListener('click',e=>{if(e.target.id==='daydetail-overlay')closeDayDetail();});
window.addEventListener('open-day-detail',e=>openDayDetail(e.detail.dayIdx));

window.addEventListener('router-close-overlay',e=>{
  if(e.detail==='locations')closeLocationModal({fromPop:true});
  if(e.detail==='daydetail')closeDayDetail({fromPop:true});
});
...
(async function boot(){
  if(state.locations.length){
    ...
    syncHeader();refreshCurrent();syncGeoWatch();
