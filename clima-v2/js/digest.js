// "El día en palabras": observaciones en lenguaje natural a partir del
// pronóstico. No es IA — son reglas sobre los datos. Devuelve una lista
// ordenada por prioridad; quien la muestra decide cuántas usar y cómo
// (la primera funciona como titular, el resto como detalle).
import {state,agroOn} from './state.js';
import {nowHourIndex,todayStartIndex,todayDailyIndex} from './weather.js';
import {temp,wind,rain,fmtHour} from './format.js';
...
export function buildDigest(data){
  ...
  // ── Lluvia en las próximas 24 h ──
  {
    ...
    for(let i=now;i<end;i++){
      ...
      if(wet){if(s<0)s=i;e=i;mm+=h.precipitation[i]||0;}
  ...
  // ── Temperatura: amplitud del día y comparación con ayer ──
  {
    ...
    const yHi=maxOf(h.temperature_2m, day0-24, day0-1);
  ...
  if(agroOn()){
    ...
    // ── Helada esta noche ──
    const frostList=frostByDay(data,state.alerts.frostThreshold);
    const frost=frostList[di+1]||frostList[di];
    ...
    // ── Condición actual para la tarea activa ──
    const fieldsAt=buildFields(data);
    const sc=scoreHour(fieldsAt,prof,now);
    if(sc.state==='verde') add(prof.icon,`Condiciones favorables ahora para ${prof.name.toLowerCase()}.`,'ok',3);
    else{
      const r=sc.reasons[0];
      add(prof.icon,`${statusLabel(sc.state)} para ${prof.name.toLowerCase()} ahora${r?` — limita ${r.label.toLowerCase()} (${r.detail})`:''}.`,sc.state==='rojo'?'warn':'',3);
    }
  }
...
export function dayDigest(data,dayIdx){
  ...
  if(agroOn()){
    ...
    const midI=Math.min(e-1,s+13);
    const midDt=deltaT(h.temperature_2m[midI],h.relative_humidity_2m[midI]);
    add('et0',`Delta-T al mediodía ~${midDt.toFixed(1)} (${midDt>=2&&midDt<=8?'ideal':midDt<2?'muy bajo':'alto'}).`, (midDt<2||midDt>8)?'warn':'ok',2);
    const fieldsAt=buildFields(data);
    const sc=scoreHour(fieldsAt,prof,midI);
    add(prof.icon,`${statusLabel(sc.state)} para ${prof.name.toLowerCase()} ese día.`,sc.state==='rojo'?'':'ok',6);
  }
